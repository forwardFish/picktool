...
export function ArchiveListClient() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('error' in payload && payload.error ? payload.error : '无法加载存档。');
      setItems((payload as ArchiveListPayload).items);
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : '无法加载存档。');
    } finally {
      setIsLoading(false);
    }
  }
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-6">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold">已保存方案</h1>
              <p className="text-slate-400">本地 MVP 存档。默认使用当前服务进程内的内存数据。</p>
            </div>
          </div>

          {isLoading ? <div className="flex items-center gap-3 text-slate-300"><Loader2 className="size-5 animate-spin" /> 正在加载已保存方案...</div> : null}
          ...
          {!isLoading && !items.length ? (
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-10 text-center text-slate-300">
              <p className="text-xl font-semibold text-white">还没有保存方案。</p>
              ...
              <article key={item.id} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-3 text-xs text-slate-500">保存于 {new Date(item.createdAt).toLocaleString()}</p>
                  </div>
                  <div className="flex gap-2">
                    <Link href={`/archive/${encodeURIComponent(item.id)}`} className="rounded-xl bg-violet-600 px-4 py-2 font-semibold text-white">打开</Link>
                    <button type="button" onClick={() => remove(item.id)} className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-slate-200" aria-label="删除存档"><Trash2 className="size-4" /></button>
                  </div>
                </div>
              </article>
            ))}
